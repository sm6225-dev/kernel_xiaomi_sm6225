...
static void msm_restart_prepare(const char *cmd)
{
	...
	if (cmd != NULL) {
		if (!strncmp(cmd, "bootloader", 10)) {
			...
			/* set flag in PMIC to notify BL post watchdog reboot */
			...
			/* set flag in PMIC to notify BL post pmic watchdog reboot */
			qpnp_pon_store_extra_reset_info(RESET_EXTRA_POST_REBOOT_MASK,
				RESET_EXTRA_POST_PMICWDT_REASON);
			/* force cold reboot */
			qpnp_pon_system_pwr_off(PON_POWER_OFF_HARD_RESET);
		} else if (!strncmp(cmd, "post-panic", 10)) {
			/* set flag in PMIC to notify BL post panic reboot */
			qpnp_pon_store_extra_reset_info(RESET_EXTRA_POST_REBOOT_MASK,
